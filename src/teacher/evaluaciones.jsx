...
export default function TeacherEvaluaciones() {
    ...
    const agruparEvaluaciones = (lista) => {
        ...
        if (Object.keys(agrupadas).length > 0) {
            ...
            const newM = {}, newS = {}, newR = {};
            Object.keys(agrupadas[pc]).forEach(sem => {
                Object.keys(agrupadas[pc][sem]).forEach(mat => {
                    const mKey = `${pc}|${sem}|${mat}`;
                    newM[mKey] = true;
                    Object.keys(agrupadas[pc][sem][mat]).forEach(sec => {
                        const sKey = `${mKey}|${sec}`;
                        newS[sKey] = true;
                        Object.keys(agrupadas[pc][sem][mat][sec].rubricas).forEach(rub => {
                            newR[`${sKey}|${rub}`] = true;
                        });
                    });
                });
            ...
            setExpandedSecciones(newS);
            setExpandedRubricas(newR);
        }
    };
                                ...
                                <div style={{ padding: '20px' }}>
                                    {Object.keys(evaluacionesAgrupadas[carrera]).map(semestre => (
                                        <div key={semestre} style={{ marginBottom: '20px' }}>
                                            <h3 style={{ color: '#334155', borderBottom: '1px solid #e2e8f0', paddingBottom: '8px', marginBottom: '15px' }}>{semestre}</h3>
