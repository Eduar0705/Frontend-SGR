...
export default function Docentes() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            let result;
            if (isEditMode) {
                result = await docentesService.updateDocente(formData.cedula_og, formData);
            } else {
                result = await docentesService.saveDocente(formData);
            }

            if (result.success) {
                Swal.fire('Éxito', result.message || 'Operación realizada correctamente', 'success');
                setShowFormModal(false);
                cargarProfesores();
            } else {
                Swal.fire('Error', result.message || 'No se pudo completar la operación', 'error');
            }
        } catch {
    ...
    const openEditModal = (profe) => {
        ...
        setFormData({
            ...
            email: profe.email, telefono: profe.telefono || '', 
    ...
    const handleDelete = (cedula) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const res = await docentesService.deleteDocente(cedula);
                    if (res.success) {
                    ...
                    <div className="modal-add" onClick={e => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleSubmit}>
                            ...
                            <div className="form-group">
                                ...
                                <input type="email" name="email" value={formData.email} onChange={handleChange} required placeholder="profesor@ejemplo.com" />
                            </div>
                            <div className="form-group">
                                <label>Teléfono:</label>
                                <input type="tel" name="telefono" value={formData.telefono} onChange={handleChange} placeholder="04145000000" />
                            </div>
                            <div className="form-group">
                                ...
                                <select name="especialidad" value={formData.especialidad} onChange={handleChange} required>
                                    ...
                                    <option value="magister">Magíster/Maestría</option>
                            ...
                            <div className="form-group">
                                ...
                                <textarea name="notas" value={formData.notas} onChange={handleChange} placeholder="Información adicional sobre el profesor..." />
                            </div>
                            <div className="form-actions">
                                <button type="button" className="btn btn-secondary" onClick={() => setShowFormModal(false)}>Cancelar</button>
                                <button type="submit" className="btn btn-primary">{isEditMode ? 'Actualizar Profesor' : 'Guardar Profesor'}</button>
                            </div>
                        </form>
                    </div>
                </div>
