...
export default function Rubricas() {
    ...
    const imprimirRubrica = (rubrica, criterios) => {
        const ventanaImpresion = window.open('', '_blank', 'width=1200,height=900');
        if (!ventanaImpresion) {
            return Swal.fire('Error', 'Ventanas emergentes bloqueadas', 'error');
        }

        // Determinar nombres de niveles para el encabezado (basado en el primer criterio con niveles)
        ...
        const html = `
            ...
            <head>
                ...
                <style>
                    ...
                    .title { flex: 1; text-align: center; font-weight: bold; font-size: 16px; text-transform: uppercase; }
                    .info-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 5px; margin-bottom: 15px; border: 1px solid #000; padding: 10px; }
                    .info-item { display: flex; border-bottom: 1px solid #eee; padding: 3px; }
                    .label { font-weight: bold; width: 140px; background: #f0f0f0; padding-left: 5px; }
                    .rubrica-table { width: 100%; border-collapse: collapse; margin-top: 15px; }
                    .rubrica-table th, .rubrica-table td { border: 1px solid #000; padding: 6px; text-align: left; vertical-align: top; }
                    .rubrica-table th { background: #2c3e50; color: white; text-align: center; }
                    .puntaje { font-weight: bold; color: #d35400; font-size: 10px; margin-top: 4px; display: block; }
            ...
            <body>
                ...
                <div class="info-grid">
                    <div class="info-item"><span class="label">Docente:</span> ${rubrica.docente_nombre}</div>
                    <div class="info-item"><span class="label">Unidad Curricular:</span> ${rubrica.materia_nombre}</div>
                    <div class="info-item"><span class="label">Carrera:</span> ${rubrica.carrera_nombre}</div>
                    <div class="info-item"><span class="label">Sección:</span> ${rubrica.seccion_codigo}</div>
                    <div class="info-item"><span class="label">Puntos:</span> ${rubrica.porcentaje_evaluacion}%</div>
                    <div class="info-item"><span class="label">Fecha:</span> ${new Date(rubrica.fecha_evaluacion).toLocaleDateString('es-ES')}</div>
                </div>
                ${rubrica.competencias ? `<div style="margin-bottom: 15px; border: 1px solid #000; padding: 10px;"><strong>Competencias:</strong><br/>${rubrica.competencias}</div>` : ''}
                <table class="rubrica-table">
                    ...
                    <tbody>
                        ${criterios.map(c => `
                            <tr>
                                <td><strong>${c.descripcion}</strong><br/><span class="puntaje">Max: ${c.puntaje_maximo} pts</span></td>
                                            ...
                                            <tr key={r.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                                ...
                                                <td style={{ padding: '15px', textAlign: 'center' }}>
                                                    <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
                                                        <button onClick={() => handleVerRubrica(r.id)} className="btns" style={{ background: '#94a3b8', color: 'white', padding: '8px', borderRadius: '8px' }} title="Ver/Imprimir">
